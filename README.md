...
- tmux pane/window naming that recognizes Codex, Claude, Kimi, DeepSeek, Gemini/Antigravity, Qwen, OpenCode, Aider, Goose, GitHub Copilot, Amp, Cursor Agent, Kiro, and Mistral Vibe across native binaries, npm/Node/package-manager launchers, and same-process-group MCP helpers instead of displaying a generic runtime label. DeepSeek-backed Claude wrappers are identified without exposing their API keys. Detection is covered on Linux and macOS's system Bash 3.
